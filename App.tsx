...
const App: React.FC = () => {
  ...
  const handleExportSelected = () => {
    if (selectedTweets.size === 0) {
      alert('Please select tweets to export');
      return;
    }
    ...
    <div className="w-[400px] h-[500px] bg-gray-900 text-gray-100 font-sans p-4 flex flex-col">
      <header className="flex justify-between items-center pb-3 border-b border-gray-700">
        ...
        <div className="flex gap-2">
          <button onClick={loadStats} className="p-2 rounded-full hover:bg-gray-800 transition-colors" aria-label="Refresh">
            <RefreshIcon className="w-5 h-5" />
          </button>
          <button onClick={() => setShowSettings(!showSettings)} className="p-2 rounded-full hover:bg-gray-800 transition-colors" aria-label="Settings">
      ...
      {showSettings ? (
        ...
        <main className="flex-grow flex flex-col mt-4 overflow-hidden">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-semibold">Tweets ({tweets.length})</h2>
            <button onClick={() => setShowTweets(false)} className="text-sm text-gray-400 hover:text-white">Back</button>
          </div>
          
          <div className="flex gap-2 mb-3">
            <button onClick={selectAll} className="text-xs px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded">Select All</button>
            <button onClick={deselectAll} className="text-xs px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded">Deselect All</button>
            <button onClick={handleExportSelected} disabled={selectedTweets.size === 0} className="text-xs px-2 py-1 bg-sky-600 hover:bg-sky-700 rounded disabled:opacity-50 disabled:cursor-not-allowed ml-auto">
              Export Selected ({selectedTweets.size})
            </button>
          </div>
          ...
          <div className="space-y-4 overflow-y-auto">
            ...
            <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
              ...
              <div className="flex items-center justify-between">
                <span className="text-gray-300">{storageMode === 'local' ? '📁 Local Only' : '☁️ Supabase + Local'}</span>
                <span className={`text-xs px-2 py-1 rounded ${storageMode === 'local' ? 'bg-yellow-900 text-yellow-300' : 'bg-green-900 text-green-300'}`}>{storageMode === 'local' ? 'Offline' : 'Synced'}</span>
              </div>
              <p className="text-xs text-gray-500 mt-2">{storageMode === 'local' ? 'Configure Supabase in settings to enable cloud sync' : 'Data is backed up locally and synced to Supabase'}</p>
            </div>

            <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
              <h2 className="text-lg font-semibold mb-3">Actions</h2>
              <div className="space-y-2">
                <button onClick={() => setShowTweets(true)} className="w-full px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-md transition-colors">View & Select Tweets</button>
                <button onClick={handleExportAll} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-700 rounded-md transition-colors">
                  <DownloadIcon className="w-4 h-4" />Export All to JSON
                </button>
                <button onClick={handleClear} className="w-full px-4 py-2 bg-red-600 hover:bg-red-700 rounded-md transition-colors">Clear Local Data</button>
              </div>
            </div>
          </div>
        </main>
